...
def test_a_proof_of_this_run_verifies(authority, anchor, results):
    proof = proof_for(authority, statement(results))

    verdict = verify_proof(proof, anchor, expectations(results))

    assert verdict.verified, verdict.failures
    assert "Results are exactly the bytes the workload attested to" in verdict.checks


def test_editing_the_results_afterwards_is_caught(authority, anchor, results):
    proof = proof_for(authority, statement(results))
...
def test_adding_a_result_file_afterwards_is_caught(authority, anchor, results):
    proof = proof_for(authority, statement(results))
...
def test_editing_the_statement_afterwards_is_caught(authority, anchor, results):
...
def test_a_proof_from_another_authority_is_refused(anchor, results):
...
def test_a_different_script_is_caught(authority, anchor, results):
    ...
    verdict = verify_proof(
        proof, anchor, expectations(results, script_image_hash="sha256:otherscript")
    )
...
def test_a_different_dataset_is_caught(authority, anchor, results):
    proof = proof_for(authority, statement(results))

    verdict = verify_proof(
        proof, anchor, expectations(results, data_hash="a-different-dataset")
    )
...
def test_an_unknown_statement_version_is_refused(authority, anchor, results):
...
def test_a_proof_outlives_the_token_that_carries_it(authority, anchor, results):
...
def test_a_proof_can_be_checked_without_the_result_files(authority, anchor, results):
    """Anyone verifying an execution they did not run has no results to hash;
    everything else still checks"""
    proof = proof_for(authority, statement(results))

    verdict = verify_proof(proof, anchor, expectations(results_dir=None))

    assert verdict.verified, verdict.failures
    assert not any("Results are exactly" in check for check in verdict.checks)


def test_every_failure_is_reported_not_just_the_first(authority, anchor, results):
    ...
    verdict = verify_proof(
        proof,
        anchor,
        expectations(results, script_image_hash="sha256:otherscript"),
    )
