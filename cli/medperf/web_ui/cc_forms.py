...
from medperf_cc.backends import section
...
def capability_settings(cc_config: Optional[dict], capability: str) -> dict:
    """What one capability is currently configured with, shared level included."""
    return section(cc_config or {}, capability)


def selected_backend(cc_config: Optional[dict], capability: str = None) -> str:
    """Which backend a stored configuration is using, if any."""
    if capability:
        return capability_settings(cc_config, capability).get(BACKEND_FIELD, "")
