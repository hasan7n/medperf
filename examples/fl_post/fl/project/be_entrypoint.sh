PYTHONSCRIPT="import torch; torch.tensor([1.0, 2.0, 3.0, 4.0]).to('cpu')"

if [ "$1" = "start_aggregator" ] || [ "$1" = "generate_plan" ]; then
    # no need for gpu, don't test cuda
    python /mlcube_project/mlcube.py $@
else
	echo "Testing torch tensors can be written to cpu (quick change, lots of output is incorect here)"
